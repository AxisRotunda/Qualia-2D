        ...
        <div class="flex items-center justify-between shrink-0">
          <div class="space-y-3">
            <div class="flex items-center gap-3">
               <div class="w-8 h-8 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-black text-xs shadow-[0_0_20px_rgba(99,102,241,0.5)]">LIB</div>
               <h3 class="text-4xl font-black text-white uppercase tracking-tighter">Object Library</h3>
            </div>
            <div class="flex items-center gap-2">
               <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
               <p class="text-[9px] text-slate-500 font-black uppercase tracking-[0.4em]">Asset Registry: <span class="text-white">{{ blueprints.length }} AVAILABLE</span></p>
            </div>
          </div>
              ...
              <button (click)="spawn.emit(bp.id); close.emit()" 
                ...
                <div class="mt-6 flex items-center justify-between relative z-10 border-t border-white/5 pt-4 opacity-40 group-hover:opacity-100 transition-opacity">
                   <div class="flex gap-2">
                      <span class="text-[7px] font-black uppercase" [class.text-amber-500]="bp.components.physics?.type === 'dynamic'" [class.text-slate-600]="bp.components.physics?.type !== 'dynamic'">
                        {{ bp.components.physics?.type === 'dynamic' ? 'Dynamic' : 'Static' }}
                      </span>
