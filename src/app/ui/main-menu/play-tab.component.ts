...
@Component({
  ...
  standalone: true,
  template: `
    <div class="h-full overflow-y-auto p-8 md:p-16 custom-scrollbar animate-in fade-in slide-in-from-right-8 duration-700">
      <header class="mb-16">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
        @for (scene of scenes; track scene.id) {
          <button (click)="select.emit(scene)" 
            class="group relative aspect-[16/9] md:aspect-auto md:h-80 p-10 rounded-[3rem] border border-white/5 bg-slate-900/40 backdrop-blur-md text-left transition-all active:scale-[0.98] overflow-hidden flex flex-col justify-between hover:border-indigo-500/40 hover:bg-slate-900/60 shadow-2xl">
            ...
            <div class="relative z-10">
              <div class="flex items-center gap-3 mb-6">
                <div class="px-3 py-1 bg-black/40 rounded-full border border-white/5 text-[8px] font-black uppercase tracking-widest text-indigo-400">{{ scene.complexity }}</div>
              ...
              <h3 class="text-4xl font-black text-white mb-4 leading-none group-hover:translate-x-2 transition-transform duration-500">{{ scene.name }}</h3>
