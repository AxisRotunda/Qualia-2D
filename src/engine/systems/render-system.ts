...
export class RenderSystem {
  ...
  private drawHighlight(ctx: CanvasRenderingContext2D, id: number, zoom: number, time: number) {
    ...
    if (s) {
        ...
        ctx.strokeStyle = '#fff'; ctx.lineWidth = 1 / zoom; ctx.globalAlpha = 1.0;
        ctx.strokeRect(-s.width / 2, -s.height / 2, s.width, s.height);
    } else {
  ...
  private drawDragLeash(ctx: CanvasRenderingContext2D, id: number, zoom: number, t: any) {
    ...
    if (target && this.state.mode() === 'play') {
        ...
        ctx.strokeStyle = '#60a5fa'; ctx.lineWidth = 2 / zoom; ctx.setLineDash([0.1, 0.1]); ctx.stroke();
        ctx.setLineDash([]);
    }
  }
  ...
  private drawGrid(ctx: CanvasRenderingContext2D, camX: number, camY: number, zoom: number, halfW: number, halfH: number) {
    ...
    if (originScreenX >= 0 && originScreenX <= this.width) { ctx.strokeStyle = '#ef444466'; ctx.beginPath(); ctx.moveTo(originScreenX, 0); ctx.lineTo(originScreenX, this.height); ctx.stroke(); }
    if (originScreenY >= 0 && originScreenY <= this.height) { ctx.strokeStyle = '#22c55e66'; ctx.beginPath(); ctx.moveTo(0, originScreenY); ctx.lineTo(this.width, originScreenY); ctx.stroke(); }
    ctx.restore();
  }
}